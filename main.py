...
with st.sidebar:
    input_name = st.text_input('Enter a name:', value = get_random_name(data))
    year_input = st.slider('Year', min_value = 1880, max_value = 2023, value =2000)
    n_names = st.radio('Number of Names per Sex', [3,5,10])

tab1, tab2 = st.tabs(['Names over Time', 'Top Names by Year'])
with tab1:
    name_df = data[(data['name']==input_name)]
    fig = px.line(name_df, x = 'year', y = 'count', color='sex')
    st.plotly_chart(fig)
